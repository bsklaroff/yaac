 ...
 *
 * Every key here was GENERATED by this server (docs/ssh-keys.md): the user
 * never brings one, and only ever sees the public half. The private half is
 * the sealed ed25519 seed, opened only to hand a process something it can
 * sign with — the egress proxy's in-memory ssh-agent, a per-worktree agent
 * under the containerless driver, the server's own in-process agent — and
...
export const gitSshKeys = snakeCase.table('git_ssh_keys', {
  ...
  /** The 32-byte ed25519 seed, base64, sealed. */
  sealedPrivateKey: text().notNull(),
  /** The public half as one OpenSSH line, in the clear: it is what the user
   *  registers with the git host, and a listing must show it without
   *  opening anything. */
