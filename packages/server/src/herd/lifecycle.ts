...
import type { HerdChangeSource, HerdLifecycle, HerdReconcileOptions } from './contract'

/**
 * Attaching a herd to its substrate, and letting go of it again.
 *
 * Everything convergence-owning starts here — the informer caches, the
 * per-workspace status watchers, the port detector — on the rule the split
 * ends at: **whoever runs a herd reconciles**. The server's reconcile loop
...
export function createLifecycle(
  ...
  const changeListeners: ((source: HerdChangeSource) => void)[] = []

  const attachNow = async (): Promise<void> => {
    ...
    // `recordedSessions` comes back over the link rather than being read
    // here: the ACP driver needs a workspace's already-recorded
    // conversations to re-address a live agent (and to `session/load` after
    // a restart), but which conversation sits on a handle is a row, and a
    // herd never looks one up.
