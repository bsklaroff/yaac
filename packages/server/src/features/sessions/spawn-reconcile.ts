/**
 * Reconcile step that drains in-session `yaac-spawn` requests: the egress
 * proxy holds each session's `POST http://yaac.internal/spawn` open in an
 ...
 * one — a request from a session no pod matches cannot be attributed to a
...
async function reportSpawnRequest(
  ...
  const decision = await serverLink().spawnRequested({
    ...
    callerWorkspaceId: req.sessionId,
  ...
  return decision.ok
    ? { requestId: req.requestId, ok: true, sessionId: decision.workspaceId }
    : fail(decision.error)
}
