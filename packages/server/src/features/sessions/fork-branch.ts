 ...
 * written at session start and rewritten only by the claim-time re-branch
 ...
 * fallback is one herd call away (docs/plans/herd-split.md).
 */
...
async function recordedForkBranch(projectSlug: string, sessionId: string): Promise<string | null> {
  ...
  return herd().workspaces.worktreeForkFallback(projectSlug, sessionId).catch(() => null)
}
