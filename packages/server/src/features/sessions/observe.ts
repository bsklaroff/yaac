...
/**
 * The herd's half of a session listing: what the substrate can see right now.
 *
 ...
 * their opening messages — is the server's, and `listActiveSessions` is the
...
async function observeRunning(p: SessionPod): Promise<WorkspaceReport> {
  const base = emptyReport(p, 'running')
  if (!p.sessionId || !p.projectSlug) return base
  return {
    ...base,
    // Aggregate over the workspace's live agents (see status-store).
    status: readSessionStatus(p.projectSlug, p.sessionId),
    ...(readSessionWaitingSince(p.projectSlug, p.sessionId) !== undefined
      ? { waitingSinceMs: readSessionWaitingSince(p.projectSlug, p.sessionId) }
      : {}),
    agents: agentLiveness(p.projectSlug, p.sessionId),
    blockedHosts: await readBlockedHosts(p.sessionId),
 ...
 */
function observeTerminating(p: SessionPod): Promise<WorkspaceReport> {
  return Promise.resolve(emptyReport(p, 'terminating'))
}

function emptyReport(p: SessionPod, phase: 'running' | 'terminating'): WorkspaceReport {
  return {
    workspaceId: p.sessionId,
