/**
 * The review diff for a worktree: everything the agent changed in its worktree
...
/** Single-quote a value for the one shell pass `podExec` gives its `cmd`
...
/**
 * Build the `podExec` command tail:
...
/**
 * Parse the pod script's output into a WorktreeChanges. name-status is the
