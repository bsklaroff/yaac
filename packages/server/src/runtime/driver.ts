import type { WorktreeRuntime } from './contract'

/**
 * The registered `WorktreeRuntime`, and the only door to it from above the
 ...
 * `worktreeRuntime()` and names no substrate.
 *
...
let registered: WorktreeRuntime | null = null

/** Install the process's runtime, or clear it on shutdown. */
export function setWorktreeRuntime(runtime: WorktreeRuntime | null): void {
  registered = runtime
}
 ...
 */
export function worktreeRuntime(): WorktreeRuntime {
  if (!registered) {
    throw new Error(
      'No WorktreeRuntime registered. The server registers one at startup; '
      + 'a test needs installFakeWorktreeRuntime() from @yaac/test-utils.',
    )
  }
  return registered
}
...
export function hasWorktreeRuntime(): boolean {
