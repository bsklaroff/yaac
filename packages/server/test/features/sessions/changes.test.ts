...
describe('getSessionChanges', () => {
  ...
  it('coalesces identical concurrent requests into a single pod exec', async () => {
    ...
    const all = Promise.all([
      getSessionChanges('yaac-proj-abc', undefined, 'main'),
      getSessionChanges('yaac-proj-abc', undefined, 'main'),
      getSessionChanges('yaac-proj-abc', undefined, 'main'),
    ])
    release()
  ...
  it('serializes differing requests for the same session', async () => {
    ...
    await Promise.all([
      getSessionChanges('yaac-proj-abc', 'dev'),
      getSessionChanges('yaac-proj-abc', 'main'),
      getSessionChanges('yaac-proj-abc', 'release'),
    ])
  ...
  it('throws rather than reporting no changes when the run failed partway', async () => {
    ...
    await expect(getSessionChanges('yaac-proj-abc')).rejects.toThrow(/completion marker/)
  })
})
...
describe('sessionForkBranch', () => {
  ...
  it('reads the session branch’s recorded upstream', async () => {
    mockUpstream.mockResolvedValue('main')
    expect(await sessionForkBranch('/repo-a', 'sid-a')).toBe('main')
    expect(mockUpstream).toHaveBeenCalledWith('/repo-a', 'agent/sid-a')
  })

  // The changes endpoint is polled every few seconds and this spawns host git,
  // so repeat reads inside the window must not hit git again.
  it('caches per session so polling does not respawn host git', async () => {
    mockUpstream.mockResolvedValue('main')
    await sessionForkBranch('/repo-b', 'sid-b')
    await sessionForkBranch('/repo-b', 'sid-b')
    await sessionForkBranch('/repo-b', 'sid-b')
    expect(mockUpstream).toHaveBeenCalledTimes(1)
    // A different session is a different entry, not a cache hit.
    await sessionForkBranch('/repo-b', 'sid-other')
    expect(mockUpstream).toHaveBeenCalledTimes(2)
  })

  // No recorded upstream is a normal state (the pod script then falls back on
  // its own), and it must be cached too — otherwise the miss respawns git on
  // every poll for exactly the sessions that are slowest to resolve.
  it('caches a missing upstream and survives a git failure', async () => {
    mockUpstream.mockResolvedValue(null)
    expect(await sessionForkBranch('/repo-c', 'sid-c')).toBeNull()
    expect(await sessionForkBranch('/repo-c', 'sid-c')).toBeNull()
    expect(mockUpstream).toHaveBeenCalledTimes(1)

    mockUpstream.mockRejectedValue(new Error('not a git repo'))
    expect(await sessionForkBranch('/repo-d', 'sid-d')).toBeNull()
  })
})
