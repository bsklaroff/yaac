...
import { builtinSkillsDir, listBuiltinSkills } from '#lib/skills/builtin'
import { parseSkillMd, fmString } from '#lib/skills/parse'
...
describe('builtin yaac-watch-prs skill', () => {
  ...
  it('documents the same usage shape as the session-bin script', async () => {
    ...
    expect(body).toContain('yaac-watch-prs [--interval <seconds>] [--model <model>] [--prompt <text>] [--once]')
  })
})
