...
/** A constant tri-state prober, typed so it slots into classifyWorkspaces. */
...
describe('classifyWorkspaces', () => {
  ...
  it('honors the graceMs argument', async () => {
    ...
    const zeroGrace = await classifyWorkspaces([p], now(), probe('alive'), 0)
    expect(zeroGrace.stale).toHaveLength(1)
    const largeGrace = await classifyWorkspaces([p], now(), probe('alive'), 10_000)
  ...
  it('leaves the terminating bucket empty for ordinary pods', async () => {
    const result = await classifyWorkspaces([pod({})], now(), probe('alive'), GRACE_MS)
