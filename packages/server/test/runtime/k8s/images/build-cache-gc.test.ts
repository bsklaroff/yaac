...
// a `kubectl exec` into its pod or a rollout of it, and `#runtime/k8s/cluster`
// is where both live.
const mockRegistryExec = vi.hoisted(() => vi.fn())
const mockRestartRegistry = vi.hoisted(() => vi.fn())
vi.mock('#runtime/k8s/cluster', () => ({
...
import { reconcileBuildCacheGc } from '#runtime/k8s/images'
...
} from '#runtime/k8s/images/build-cache-gc'
