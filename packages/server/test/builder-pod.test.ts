...
import type * as imagePromoterModule from '#lib/container/image-promoter'
import type * as registryServiceModule from '#lib/k8s/registry-service'
...
vi.mock('#lib/container/image-promoter', async (importOriginal) => ({
...
vi.mock('#lib/k8s/vcluster', () => ({ vapAvailable: mockVapAvailable }))

const mockEnsureRegistryClusterService = vi.hoisted(() => vi.fn())
vi.mock('#lib/k8s/registry-service', async (importOriginal) => ({
...
} from '#lib/container/builder-pod'
import type { ImageLayer } from '#lib/container/image-builder'
