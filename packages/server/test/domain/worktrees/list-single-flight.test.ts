...
describe('listActiveWorktrees single-flight', () => {
  ...
  it('keeps different filters on separate in-flight slots', async () => {
    ...
    await fs.mkdir(path.join(tmpDir, 'global', 'projects', 'proj-a'), { recursive: true })
    await fs.writeFile(path.join(tmpDir, 'global', 'projects', 'proj-a', 'project.json'), JSON.stringify({
      slug: 'proj-a', remoteUrl: 'https://example.com/a.git', addedAt: '2026-01-01T00:00:00.000Z',
    }))
    await fs.mkdir(path.join(tmpDir, 'global', 'projects', 'proj-b'), { recursive: true })
    await fs.writeFile(path.join(tmpDir, 'global', 'projects', 'proj-b', 'project.json'), JSON.stringify({
