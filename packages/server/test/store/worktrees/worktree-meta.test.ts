...
describe('worktree-meta', () => {
  ...
  describe('worktreesOnCurrentLife', () => {
    it('counts only handles the current life recorded', async () => {
      ...
      expect(worktreesOnCurrentLife(first as WorktreeMeta).map((s) => s.agentSessionId))
        .toEqual(['a'])
      ...
      expect(worktreesOnCurrentLife(second as WorktreeMeta)).toEqual([])
  ...
  describe('foldSessionStarts', () => {
    ...
    it('does not re-validate the previous life\'s pane after a restart', async () => {
      ...
      expect(worktreesOnCurrentLife(meta as WorktreeMeta).map((s) => s.agentSessionId))
        .toEqual(['the-new-one'])
