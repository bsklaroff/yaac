...
// *inside* a yaac worktree (nested yaac), the worktree preset leaks
...
// in-cluster registry Service, createWorktree /
// runClusterCheck take their nested branches, and getDataDir() resolves to
// the worktree's node-shared virtiofs data dir (slow, coarse file
...
// identical on a developer host and inside a worktree; each test sets its
// own data dir via setDataDir / createTempDataDir.
//
// Only the `unit` project loads this file. E2e tests run the real CLI
// in-container and genuinely need the nested-worktree env (the server
