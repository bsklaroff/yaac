...
export async function worktreeMonitor(projectSlug?: string, options: WorktreeMonitorOptions = {}): Promise<void> {
  ...
  while (true) {
    ...
    try {
      ...
      console.log(`yaac worktree monitor  (every ${intervalSec}s, ${now})  Press Ctrl+C to exit\n`)
      await worktreeList(projectSlug)
    } finally {
