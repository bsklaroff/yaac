...
beforeEach(() => {
  vi.clearAllMocks()
  vi.mocked(renameWorktree).mockResolvedValue(undefined)
})
...
describe('WorktreeTitle', () => {
  it('shows the title as selectable text with a rename affordance', () => {
    render(<WorktreeTitle worktreeId="s1" title="My worktree" prompt="do a thing" />)
    const label = screen.getByText('My worktree')
    // Selectable for copy/paste (opts out of the Electron drag region).
    expect(label.className).toContain('select-text')
    expect(screen.getByRole('button', { name: 'Rename worktree' })).toBeTruthy()
    // Not editing yet: no input on screen.
    expect(screen.queryByRole('textbox')).toBeNull()
  })

  it('falls back to the prompt when there is no title', () => {
    render(<WorktreeTitle worktreeId="s1" title="" prompt="my first prompt" />)
  ...
  it('opens the editor pre-filled with the existing title, cursor at the end', () => {
    render(<WorktreeTitle worktreeId="s1" title="Existing name" prompt="p" />)
  ...
  it('seeds the editor from the prompt when the worktree has no title', () => {
    render(<WorktreeTitle worktreeId="s1" title="" prompt="do the thing" />)
    expect(openEditor().value).toBe('do the thing')
  })

  it('copies the title without the flex-item block boundary newlines', () => {
    render(<WorktreeTitle worktreeId="s1" title="My title" prompt="p" />)
  ...
  it('commits a rename on Enter', () => {
    render(<WorktreeTitle worktreeId="s1" title="Old" prompt="p" />)
    const input = openEditor()
    expect(input.value).toBe('Old')
    fireEvent.change(input, { target: { value: 'New name' } })
    fireEvent.keyDown(input, { key: 'Enter' })

    expect(renameWorktree).toHaveBeenCalledWith('s1', 'New name')
  ...
  it('commits a rename on blur', () => {
    ...
    expect(renameWorktree).toHaveBeenCalledWith('s1', 'Renamed')
  })
  ...
  it('does not rename when the value is unchanged', () => {
    render(<WorktreeTitle worktreeId="s1" title="Same" prompt="p" />)
    const input = openEditor()
    fireEvent.keyDown(input, { key: 'Enter' })
  ...
  it('does not persist the prompt fallback when an untitled worktree is committed unchanged', () => {
    ...
    render(<WorktreeTitle worktreeId="s1" title="" prompt="my first message" />)
    const input = openEditor()
  ...
  it('collapses a multi-line prompt fallback to one line and treats it as unchanged', () => {
    render(<WorktreeTitle worktreeId="s1" title="" prompt={'do a\n   thing'} />)
    const input = openEditor()
  ...
  it('trims surrounding whitespace before comparing and committing', () => {
    render(<WorktreeTitle worktreeId="s1" title="Kept" prompt="p" />)
    const input = openEditor()
    ...
    expect(renameWorktree).toHaveBeenCalledWith('s1', 'Fresh')
  })
})
