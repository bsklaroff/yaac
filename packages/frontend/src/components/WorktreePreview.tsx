...
/**
 * A per-worktree embedded browser pane pointed at a dev server running inside
 * the worktree pod (reached over a loopback forwarded port). The chrome —
...
export function WorktreePreview({
  ...
  worktreeId: string
  /** Forwarded ports available to preview for the worktree. */
  ports: PortMapping[]
  ...
  // Create the webview once per worktree; wire its navigation events.
  useEffect(() => {
    if (!electron) return
