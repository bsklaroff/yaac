...
import { ControlModeClient, type ControlModeNotification } from '@/daemon/control-mode'
...
export class SessionStatusWatcher {
  ...
  private connect(): void {
    ...
    try {
      ...
      this.log(`[daemon] status-watcher ${sessionId}: spawn failed: ${String(err)}`)
  ...
  private onStreamDown(generation: number, reason: string): void {
    ...
    this.log(`[daemon] status-watcher ${this.session.sessionId}: ${reason}`)
    this.teardownStream()
