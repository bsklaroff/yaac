 ...
 * the summarizer). One attempt per session per daemon run: the in-memory
...
/** Sessions already handled this daemon run; added synchronously before the
...
async function generateOne(slug: string, sessionId: string, prompt: string): Promise<void> {
  try {
    ...
    daemonLog(`[titles] ${slug}/${sessionId}: ${String(err)}`)
  }
}
