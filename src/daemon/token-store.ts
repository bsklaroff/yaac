...
import { constantTimeEqual } from '@/daemon/web-auth'
import { maskToken } from '@/shared/mask'
import { tokensPath } from '@/shared/paths'

/**
 * Durable client tokens: what a remote CLI (or the auth daemon) presents
  ...
  return {
    create: (name) => {
      ...
      if (entries.some((e) => e.name === name)) {
        throw new DaemonError('CONFLICT', `a token named '${name}' already exists — revoke it first`)
      }
