import type { AgentTool, DaemonSnapshot } from '@/shared/types'

/**
 * The attention model: turn a daemon snapshot into the "needs me" signal the
...
export function selectWaiting(snapshot: DaemonSnapshot): WaitingSession[] {
 ...
 */
export function parseSnapshotMessage(raw: string): DaemonSnapshot | null {
  try {
    ...
    if (parsed.type === 'snapshot' && parsed.data !== null && typeof parsed.data === 'object') {
      return parsed.data as DaemonSnapshot
    }
  } catch {
 ...
 * notifications), so connecting to a daemon with pre-existing waits doesn't
...
export class AttentionMonitor {
  ...
  update(snapshot: DaemonSnapshot): { waitingCount: number; toNotify: WaitingSession[] } {
