...
2. **Quantization is the size lever, and the imatrix quant wins.** For
   ...
   which mashed words and hallucinated (`Llama 2 Session Naming Optimization`).
...
- **Prompts:** 16 realistic session first-messages spanning short / medium /
...
- **Prompt template (decoder models):** system = *"You write concise, specific
  titles for a developer tool's session list."*; user = *"Write a short,
...
Prompt:
```text
can we make the session title extend the full width of the sidebar, unless the row is hovered over and then the x can take precedence (currently it always leaves room for the x). also, when hovered, can we do a marquee effect to view the entire title as it scrolls by
```

| model (GGUF key) | ms | title output |
|---|---|---|
| `flan-t5-small` | 1142 | x row |
| `flan-t5-base` | 3265 | Can we make the session title extend the full width of the sidebar? |
| `flan-t5-large` | 11258 | Can we make the session title extend the full width of the sidebar? |
...
| `qwen2.5-1.5b` | 4052 | Sidebar Session Titles: Full Width Hover Marquee |

#### `real-larger-models` — long/real _(this very request)_

Prompt:
```text
can we try some slightly larger models for row naming via llama.cpp, then choose one that consistently picks better names for sessions than the current model? in particular, we want longer prompts, or more obscure prompts, to not produce relatively uninformative or irrelevant titles
```
...
| `qwen2.5-0.5b` | 2259 | Experimenting with Larger Models for Session Naming |
| `llama3.2-1b` | 3761 | Llama Model Expansion |
| `qwen2.5-1.5b` | 3517 | Model Evaluation: Larger Inputs for Better Session Titles |
...
| `smollm2-360m` | 9851 | "5000/8443 Session Exfiltration Rule Re-Exposed: A Case for Simpl |
| `qwen2.5-0.5b` | 3390 | Cilium session rule optimization |
| `llama3.2-1b` | 4459 | Session Egress Allow Scope |
| `qwen2.5-1.5b` | 4363 | Refine Session Egress Rule Scope |

#### `long-title-summarizer` — long/obscure

Prompt:
```text
auto-generate session titles for untitled sessions by shelling out to a pinned llama.cpp completion binary running a small quantized flan-t5 gguf in a background-loop step, persisting through setSessionTitle so a user rename always overrides, with a vocabulary guard that drops hallucinated off-topic titles
```

| model (GGUF key) | ms | title output |
|---|---|---|
| `flan-t5-small` | 1417 | Untitled sessions pinned to llama.cpp |
| `flan-t5-base` | 841 | Auto-generate session titles for untitled sessions |
| `flan-t5-large` | 6950 | llama.cpp session title generator |
| `smollm2-360m` | 2268 | Session Titles for Untitled Sessions |
| `qwen2.5-0.5b` | 7748 | Quantized Flan-T5 Completion by Background Loop |
| `llama3.2-1b` | 5038 | Auto-generate session titles for pinned llama binaries |
| `qwen2.5-1.5b` | 5831 | Auto-generate Session Titles for Untitled Sessions |

#### `obscure-cilium-tproxy` — obscure/jargon

Prompt:
```text
the session pod's tproxy'd 443 egress SYN is getting dropped because src_valid_mark isn't set on the node; add the rp_filter/route_localnet node fixups in cluster-setup and assert the datapath in cluster-check
```
...
| `smollm2-360m` | 2721 | "Session Pod's Tproxy'd 443 EGPIN is Dropped: Add RpFilter/Rout |
| `qwen2.5-0.5b` | 4609 | Cluster setup fixes for dropped egress SYN on session pod |
...
Prompt:
```text
add a hover marquee to the sidebar session rows so long titles scroll into view
```

| model (GGUF key) | ms | title output |
|---|---|---|
| `flan-t5-small` | 5983 | Speaking': 'Speaking': |
| `flan-t5-base` | 2267 | Hover marquee to sidebar session rows |
| `flan-t5-large` | 10144 | add a hover marquee to the sidebar session rows so long titles scroll |
| `smollm2-360m` | 1169 | Hover-Marquee Sidebar Titles |
| `qwen2.5-0.5b` | 4671 | Session List Hover Marquee |
| `llama3.2-1b` | 3666 | Session Title |
...
Prompt:
```text
the session-create e2e test is flaky under podman — it sometimes races to build the base image inside the worker instead of using the prebuilt one; make it require the prebuilt image and fail fast
```
...
| `flan-t5-large` | 6759 | session-create e2e test |
| `smollm2-360m` | 1380 | Session-Create E2E Test: Flaky Under Podman |
| `qwen2.5-0.5b` | 1919 | Session Create E2E Test Flaky Podman |
...
| `gemma3-270m-Q8` | 6674 | Can we make the session title extend the full width of the sidebar, unless the row is hovered over and then the x can take precedence (currently it always leaves room for the x). also, when |
| `gemma3-270m-Q4KM` | 9266 | Here are a few options: |
| `lfm2-350m-Q8` | 2895 | Can we make session title sidebar full width on hover? Can we add a marquee effect for scrolling view? |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q8` | 1967 | Experimenting with Larger Models for Session Naming |
| `qwen2.5-0.5b-Q5KM` | 1415 | Experimenting with Larger Models for Session Naming via llama.cpp |
| `qwen2.5-0.5b-Q4KM` | 5351 | Llama 2 Session Naming Optimization |
| `qwen3-0.6b-Q4KM` | 5024 | Try larger row naming models with llama.cpp, prioritize consistent session naming over current model |
| `gemma3-270m-Q8` | 3794 | Can we try some slightly larger models for row naming via llama.cpp, then choose one that consistently picks better names for sessions than the current model? |
| `gemma3-270m-Q4KM` | 2464 | [] |
| `lfm2-350m-Q8` | 5640 | LONG SESSION NAME TITLE: LAMBDA MODEL NAME IMPROVEMENT |
| `lfm2-350m-Q4KM` | 2345 | Larger Model Performance: Larger Session Names |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q8` | 2782 | Cilium session rule optimization |
| `qwen2.5-0.5b-Q5KM` | 3093 | Session List: Narrow Scope to Per-Project Registry and VCluster APIservers |
| `qwen2.5-0.5b-Q4KM` | 2593 | Cilium session rule optimization |
...
| `qwen2.5-0.5b-Q5KM` | 4565 | Pinched Completion: Auto-Generate Session Titles |
| `qwen2.5-0.5b-Q4KM` | 3093 | PinLlamaCompletionSessionTitle |
| `qwen3-0.6b-Q4KM` | 17370 | 1. "Renaming in Progress" |
| `gemma3-270m-Q8` | 2091 | **llama-cpp-5-session-title** |
| `gemma3-270m-Q4KM` | 5503 | **Title:** |
| `lfm2-350m-Q8` | 2362 | auto-generate-session-title-1 |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q8` | 3234 | Cluster setup fixes for dropped egress SYN on session pod |
| `qwen2.5-0.5b-Q5KM` | 1857 | Session Pod Tproxy Issues: Fixup Node Configuration |
| `qwen2.5-0.5b-Q4KM` | 3806 | Cluster setup fixups for dropping session pod's egress SYN |
| `qwen3-0.6b-Q4KM` | 14765 | session pod tproxy 443 egress SYN dropped due to src_valid_mark missing; apply rp_filter/route_localnet fixups in cluster-setup and assert datapath in |
| `gemma3-270m-Q8` | 6410 | The session pod's tproxy'd 443 egress SYN is getting dropped because src_valid_mark isn't set on the node; add the rp_filter/route_ |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q8` | 3836 | Session List Hover Marquee |
| `qwen2.5-0.5b-Q5KM` | 2794 | Session List Hover Marquee |
| `qwen2.5-0.5b-Q4KM` | 3947 | Session Titles Scroll Into View |
| `qwen3-0.6b-Q4KM` | 5117 | Add hover marquee to sidebar session rows |
| `gemma3-270m-Q8` | 5417 | Here's a title: |
| `gemma3-270m-Q4KM` | 872 | [] |
| `lfm2-350m-Q8` | 1913 | Add hover marquee to all session rows for long titles to scroll |
| `lfm2-350m-Q4KM` | 1754 | Add Hover Marquee to Sidebar Session Rows for Long Titles |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q8` | 2491 | Session Create E2E Test Flaky Podman |
| `qwen2.5-0.5b-Q5KM` | 4309 | Session Create E2E Test Flaky Podman |
| `qwen2.5-0.5b-Q4KM` | 3244 | Session Create E2E Test Flaky under Podman |
| `qwen3-0.6b-Q4KM` | 12208 | session-create e2e test is flaky under podman — it sometimes races to build the base image inside the worker instead of using the prebuilt one; make it require the |
| `gemma3-270m-Q8` | 6987 | The session-create e2e test is flaky under podman — it sometimes races to build the base image inside the worker instead of using the prebuilt one; make it require the prebuilt image |
...
| `danube3-500m-Q5KM` | 2541 | Session Title: [Your Title] |
| `danube3-500m-Q4KM` | 2434 | The Ultimate Guide to Session Listing for Developers |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q5KM` | 3982 | Experimenting with Larger Models for Session Naming via llama.cpp |
| `qwen2.5-0.5b-Q4KS` | 2596 | Llama 2 Session Naming Optimization |
| `qwen2.5-0.5b-IQ4XS` | 1349 | Optimize Session Naming with Larger Models |
| `qwen2.5-0.5b-Q3KXL` | 1675 | Experimenting with Larger Models for Session Naming via llama.cpp |
| `danube3-500m-Q5KM` | 1225 | Sure, here are some suggestions for longer titles that could be used for a developer tool's session list: |
| `danube3-500m-Q4KM` | 4653 | Sure, here are some titles for a developer tool's session list that are concise, specific, and capture the main point of the request: |
| `smollm2-360m-Q5KM` | 1295 | Can we try some slightly larger models for row naming via llama.cpp, then choose one that consistently picks better names for sessions than the current model? |
...
| `danube3-500m-Q5KM` | 1621 | Sure, here's a concise and specific title for a developer tool's session list: |
| `danube3-500m-Q4KM` | 1579 | Sure, here's a concise and specific title for a developer tool's session list: |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q5KM` | 3268 | Session List: Narrow Scope to Per-Project Registry and VCluster APIservers |
| `qwen2.5-0.5b-Q4KS` | 1351 | Cilium session rule optimization |
| `qwen2.5-0.5b-IQ4XS` | 1610 | Cilium session rule optimization |
...
| `smollm2-360m-Q5KM` | 4455 | Session-egress allow rule: too broad, scope to per-project registry, and leverage Cilium's union-of-allows for better nested case |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q5KM` | 3699 | Pinched Completion: Auto-Generate Session Titles |
| `qwen2.5-0.5b-Q4KS` | 1210 | PinLlamaCompletionSessionTitleGenerator |
| `qwen2.5-0.5b-IQ4XS` | 954 | Pin llama.cpp completion binary for session titles |
| `qwen2.5-0.5b-Q3KXL` | 1135 | Quantized Flan-T5 Session Title Generator |
| `danube3-500m-Q5KM` | 1808 | Untitled Session Title: A User-Friendly Way to Rename Sessions |
| `danube3-500m-Q4KM` | 1384 | Title: 'Session Title' - A concise, specific title that captures the main point of this request |
| `smollm2-360m-Q5KM` | 1737 | Session Titles: Auto-Generating Session Titles for Untitled Sessions |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q5KM` | 3868 | Session Pod Tproxy Issues: Fixup Node Configuration |
| `qwen2.5-0.5b-Q4KS` | 3090 | Cluster setup fixes for dropped egress SYN traffic |
| `qwen2.5-0.5b-IQ4XS` | 1230 | Session pod's Tproxy'd 443 Egress SYN Drops, Fixups Required |
| `qwen2.5-0.5b-Q3KXL` | 1624 | Session Pod Tproxy Dropped SYN: Fixup Cluster Setup and Check |
| `danube3-500m-Q5KM` | 1285 | The session pod's tproxy'd 443 egress SYN is getting dropped because src_valid_mark isn't set on the node; add the rp_filter |
| `danube3-500m-Q4KM` | 1719 | The session pod's tproxy'd 443 egress SYN is getting dropped because src_valid_mark isn't set on the node; add the rp_filter |
| `smollm2-360m-Q5KM` | 4314 | "Session Pod's Tproxy'd 443 Egress SYN is Dropped: Add RpFilter/RouteLocalNet Fixups in ClusterSetup and Assert Datapath in Cluster |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q5KM` | 1358 | Session List Hover Marquee |
| `qwen2.5-0.5b-Q4KS` | 1596 | Session List Hover Marquee |
| `qwen2.5-0.5b-IQ4XS` | 934 | Session List Hover Marquee |
| `qwen2.5-0.5b-Q3KXL` | 2875 | Enhance Sidebar Session Rows with Hover Marquee |
...
| model (GGUF key) | ms | title output |
|---|---|---|
| `qwen2.5-0.5b-Q5KM` | 1602 | Session Create E2E Test Flaky Podman |
| `qwen2.5-0.5b-Q4KS` | 1655 | Session Create E2E Test Flaky under Podman |
| `qwen2.5-0.5b-IQ4XS` | 954 | Session Create E2E Test Flaky Podman Build Race |
| `qwen2.5-0.5b-Q3KXL` | 2167 | Session Create E2E Test Flaky Podman Build Race |
| `danube3-500m-Q5KM` | 1491 | The session-create e2e test is flaky under podman because it sometimes races to build the base image inside the worker instead of using the prebuilt one |
| `danube3-500m-Q4KM` | 2918 | The session-create e2e test is flaky under podman because it sometimes races to build the base image inside the worker instead of using the prebuilt one. Make it require the prebuilt |
| `smollm2-360m-Q5KM` | 2838 | Session-Create E2E Test: Flaky Under Podman, Require Prebuilt Image, Fail Fast |
