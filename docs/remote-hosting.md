...
`127.0.0.1` — nothing in the CLI or webapp assumes the daemon is on the
same machine.
...
# 2. Join the tailnet and serve the daemon over TLS — tailnet-only
#    (`serve`, never `funnel`):
tailscale up
tailscale serve --bg https / http://127.0.0.1:8787

# 3. Tell the daemon to admit its tailnet hostname and trust the proxy's
#    TLS signal (put these in the daemon's environment permanently, e.g.
...
`yaac remote off` switches back to a local daemon without forgetting the
...
Everything goes through the daemon, so the CLI and webapp behave the same
against a local or remote daemon:

- Sessions: create, list, attach, shell, stream, restart, delete — the
  terminal rides the daemon's PTY WebSocket (`C-b d` detaches, exactly like
  a local attach).
- Config editing: `yaac config edit*` fetches the file from the daemon,
  opens your local `$EDITOR`, and saves back through the daemon's
  validation.
- Credentials: `yaac auth update` runs Claude/Codex browser sign-ins **on
  your machine** (via the auto-started auth daemon — the broker that owns
  the vendor login CLIs) and ships the captured bundle to the server. The
  webapp's sign-in cards drive the same flow; if no auth daemon is running
  they say what to start.

Semantics to keep in mind:

- **Paths are daemon-host paths.** `yaac session create --add-dir <path>`
...
- **Machine-scoped commands** operate wherever they run and ignore the
  remote setting: `yaac daemon *`, `yaac cluster *`,
  `yaac auth daemon *` (the auth daemon is by design the local machine's
  broker).
- **Phone-only clients can't mint tool credentials** — sign-in needs the
  auth daemon, which needs the CLI. Set credentials up from a laptop once;
  everything else works from the phone.
- A version mismatch between client and server prints a one-time warning
  (the daemon reports its build id on every response); upgrade whichever
  side is behind.
...
- **Remote access is opt-in and default-safe.** Without
  ...
  daemon is exactly as loopback-only as before.
...
- **Reboot durability** (systemd unit for the daemon, cluster restart on
  boot) — run `yaac cluster setup --repair && yaac daemon start` after a
  server reboot for now; see `plans/remote-daemon-hosting.md` (Layer 3).
