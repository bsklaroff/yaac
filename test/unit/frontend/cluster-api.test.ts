...
import { getClusterCheck, streamClusterSetup } from '@/frontend/lib/clusterApi'
...
describe('getClusterCheck', () => {
  it('returns the daemon check result', async () => {
...
describe('streamClusterSetup', () => {
  ...
  it('throws the daemon error message on an error event', async () => {
    globalThis.fetch = vi.fn().mockResolvedValue({
      ok: true,
      status: 200,
