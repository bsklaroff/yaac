...
vi.mock('@/daemon/title-summarizer', () => ({
...
} from '@/daemon/title-generation'
import { listActiveSessions } from '@/lib/session/list'
import { setSessionTitle } from '@/lib/session/titles'
import { shouldGenerateTitle, summarizeTitle } from '@/daemon/title-summarizer'
...
describe('title generation', () => {
  ...
  it('attempts each session once per daemon run (in flight, done, or cleared)', async () => {
    listOf(session())
    await reconcileGeneratedTitles()
    await flush()
  ...
  it('swallows a session-list failure', async () => {
    mockList.mockRejectedValue(new Error('daemon starting'))
