...
- Cluster access uses the `@kubernetes/client-node` library wherever a library call applies (reads, informers/watches); `kubectl exec` is used only where a library call doesn't make sense — streaming into session pods (PTYs, port-forward relays, tmux control-mode status). Primitive helpers live in `packages/server/src/drivers/k8s/substrate/` (sealed behind `#drivers/k8s/substrate`, the k8s driver's own bottom); cluster lifecycle (setup/check/delete) lives behind the `#drivers/k8s/cluster` barrel. The local OCI registry is a host podman container with no Kubernetes object in it, so it sits in `packages/server/src/drivers/k8s/container/registry.ts` beside the container runtime, and the datapath's names and ports are a zero-import vocabulary in `packages/server/src/drivers/k8s/substrate/proxy-constants.ts`.
