...
- A worktree's agents run in one of two **modes** (`docs/agent-modes.md`): `tui` (the tool's terminal UI, observed through tmux control mode, rendered by xterm.js) or `acp` (the tool's Agent Client Protocol adapter, driven over JSON-RPC, rendered as a chat pane). Both launch into a tmux window — tmux is the process supervisor that outlives the viewer either way; only the presentation transport differs. `agentDriver(mode)` in `#features/agents` is the seam, and `WorktreeStatusWatcher` owns the retry policy for both. An ACP agent is supervised in-pod by `acpd` (`dockerfiles/acpd/`, baked into the base image) so a dropped connection never kills a running turn.
