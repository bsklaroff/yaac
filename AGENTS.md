...
- Tests that can't run inside a nested yaac session (in-cluster Cilium datapath assertions, vcluster-in-vcluster, podman `kind` network) are gated on `IS_NESTED_YAAC` (`test/helpers/setup.ts`) via `describe.skipIf` / `it.skipIf`. The session-create e2e family (own daemon+proxy+mocks) runs nested ungated — it assumes the outer daemon projects per-install inner redirects (docs/yaac-in-yaac-inner-egress.md); egress timeouts nested mean the host yaac predates that and needs upgrading.
