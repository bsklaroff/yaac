...
interactive *CLI* commands onto that bridge (so the daemon becomes
...
- **Auth bootstrap** (`src/daemon/web-auth.ts`, `src/daemon/server.ts`).
  ...
  - **TTL is 24h, not 60s.** The code is single-use and 256-bit and
    is already retrievable from `yaac daemon logs` for the daemon's
  ...
  - **Added `GET /auth/bootstrap-code`** (authenticated): `yaac open`
    ...
    ready-to-open URL, so nothing scrapes the daemon log.
  - **Multi-session persistence.** Minted sessions are capped FIFO
    (`MAX_SESSIONS`) and persisted across daemon restarts so a
    rebuild doesn't force every open browser to re-bootstrap.
- **Static asset serving + build wiring** (`src/daemon/static.ts`,
  ...
  `dist/frontend/`; the daemon registers the static routes when that
  bundle is present. `pnpm frontend:dev` runs Vite and proxies the
  daemon API + `/events` + `/pty` over to the bound daemon port.
- **Request gating** (`src/daemon/web-auth.ts`, `src/daemon/auth.ts`).
...
`src/daemon/cli.ts`, not in `buildApp`, so `buildApp`'s return type
...
- **Host-header check.** Reject if `Host`'s *hostname* isn't
  ...
  - *Simplification vs. the original sketch:* only the hostname is
    ...
    the daemon from outside its container) legitimately remaps the
    external port, so the browser's `Host` port need not equal the
    daemon's bound port. The port comparison adds no real defense
    anyway — a rebind request must already target the daemon's real
    port to connect.
- **CORS.** Implemented as deny-all-browser-CORS: preflight
  ...
  - *Simplification vs. the original sketch:* the webapp is
    same-origin with the daemon, so it never needs CORS at all.
    Rather than reflect same-origin in ACAO, the daemon refuses
...
Shipped (`src/daemon/web-auth.ts`, `src/daemon/server.ts`,
`src/daemon/cli.ts`). Browsers can't read the bearer out of the lock
file, so a one-time exchange bridges them in:

1. Daemon startup generates a 256-bit `bootstrapCode` and logs it as
...
step 2. The daemon never logs the code value — only `bootstrap ok` /
`bootstrap fail`.

`yaac open` (`src/daemon/cli.ts`) skips the log entirely: it ensures
the daemon is up, fetches a fresh code over the bearer-authed
`GET /auth/bootstrap-code`, prints the authed URL (scriptable), and
launches a browser.

## Static asset serving

Shipped (`src/daemon/static.ts`, `vite.config.ts`).
...
- In dev, `pnpm frontend:dev` runs Vite on its own port and proxies
  the daemon API + `/events` + `/pty` to the bound daemon port.
  Production is daemon-only.

Build wiring: `pnpm build` runs `tsup && vite build`, copies
`dockerfiles` + `k8s` into `dist/`, and emits the SPA into
`dist/frontend/` (Vite `outDir`). The daemon resolves the static dir
...
the daemon's JSON `404`. The webapp keeps its routing client-side, so
...
One WebSocket: `/events` (`src/daemon/cli.ts`, `src/daemon/events.ts`).
The daemon emits a single message type:

- `snapshot` — the full daemon state the webapp hydrates from
  (`DaemonSnapshot`: active sessions, stale sessions, projects, and
...
Shipped (`src/daemon/cli.ts`, `src/daemon/pty-bridge.ts`).

Each terminal tab in a client is one PTY on the daemon side, exposed
...
- Daemon restart → all sockets close; on reconnect the client
  ...
  because tmux runs inside the container, which outlives the daemon.
...
- `WS /session/stream`: move the `pickNextStreamSession` state
  ...
  `src/daemon/routes/session.ts`, logic in `src/daemon/stream-picker.ts`)
  behind a WS. The daemon picks the next waiting session (or creates
  one), opens a PTY, streams it; on detach it picks the next; on "no
  more" it closes. The CLI side becomes a pure pump.
- `WS /auth/update`: run the interactive flow on the daemon,
  including the `claude login` / `codex login` shell-outs. Simple
  prompts travel as control messages; tool-login PTYs travel as
  binary frames. After this lands, the daemon is the only process
  that invokes those login CLIs, so credential bundles never transit
  the CLI process.

Net effect of this item: the daemon becomes the **only** process
...
worktree in editor" needs a daemon endpoint:
...
- **Unit**: `test/unit/daemon/events.test.ts` (snapshot hub: connect
  sends snapshot, diff-suppressed re-broadcast),
  `test/unit/daemon/pty-bridge.test.ts` (control-message parsing,
  target parsing, bridge wiring), `test/unit/daemon/web-auth.test.ts`
  (`/auth/bootstrap`: valid → cookie; reused → null; expired → null;
  mismatch → null; host-header allow/deny),
  `test/unit/daemon/open-webapp.test.ts`,
  `test/unit/frontend/bootstrap.test.ts`.
- **API/E2E**: `test/api/bootstrap-flow.test.ts` (code → cookie →
  authed request; reuse 401), `test/e2e-cli/daemon-ws.test.ts`
...
7. `WS /auth/update` + port `yaac auth update` onto it. Tool-login
   shell-outs (`claude login`, `codex login`) move inside the daemon.
...
4. **Bootstrap code rotation.** Single-use means the daemon mints a
