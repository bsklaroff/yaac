...
- A single **server** (home box or cloud VM) runs the *entire* yaac stack:
  the kind cluster, podman, and the daemon. Sessions are Kubernetes Jobs on
...
- The **laptop, phone, and tablet are thin clients.** The CLI talks to the
  remote daemon; the webapp is just a browser pointed at the server. Closing
...
daemon's loopback posture and the CLI's hardcoded transport. Two changes clear
them:
...
bearer secret in `~/.yaac/.daemon.lock`, which is regenerated on every daemon
start (`src/daemon/cli.ts:137`) and read fresh from disk on every call
(`src/shared/daemon-client.ts:135`). A laptop has no access to that file and
it rotates on every restart anyway. The webapp already sidesteps this with the
bootstrap-code → HttpOnly-cookie flow (`src/daemon/web-auth.ts`); the CLI has
...
```
   ...
   │   yaac daemon  ─ listens 127.0.0.1:8787 ─┐    │
...
**Why `tailscale serve` rather than binding the daemon to the tailnet:**

- The daemon keeps its **loopback bind unchanged** (`src/daemon/cli.ts:100`) —
  the smallest, safest possible server-side change. Serve proxies the tailnet
  to `127.0.0.1`; nothing else can reach the daemon.
...
- **Tailnet-only.** Serve (not Funnel) never exposes the daemon to the public
...
Three independently shippable layers. The first is pure client/daemon code and
...
- `src/daemon/web-auth.ts`: extend `cookieOrBearerAuth` so a request is
...
- `src/daemon/routes/auth.ts`: add `POST /auth/tokens` (create, returns the
...
- `src/shared/daemon-client.ts` — the central refactor. Today
  `defaultResolveLock` returns a `DaemonLock` and `createDaemonFetch`
  hardcodes `http://127.0.0.1:${lock.port}` (`:44`). Generalize the resolved
  shape to a **`DaemonTarget` = `{ baseUrl, secret, buildId? }`** and migrate
  ...
  - the existing `YAAC_DAEMON_URL` + `YAAC_DAEMON_SECRET` env hatch (`:137`)
    folds into this as the highest-precedence override.
  `createDaemonFetch` then targets `target.baseUrl + pathAndSearch`. The
...
- **`yaac open` becomes context-aware.** Against a remote context it fetches
  ...
  lock (`src/daemon/cli.ts:435`); generalize the host from the active target.
- **Build-id skew:** `describeLockMismatch` compares the local lock's buildId
  to the CLI's (`src/shared/daemon-client.ts:106`). For a remote target there
  is no lock; read `buildId` from the daemon's `/health` (it already returns
  it — `src/daemon/server.ts:108`) and **warn** on mismatch rather than
...
If the CLI has named contexts, how does the **webapp** choose which daemon to
talk to? It doesn't, and structurally can't the way the CLI does — by design:

- The SPA is **served by the daemon and bound to that origin.** Every call is
...
- So "which daemon" is decided entirely by **which URL you load**:
  `http://127.0.0.1:8787/` is the local daemon; `https://srv.<tailnet>.ts.net/`
...
daemons; the webapp is the mirror image — many SPA instances, each pinned to
one daemon. The two selectors are **parallel, not coupled**: `yaac context use
...
A true *in-app* daemon switcher (a dropdown that calls a different daemon's
...
only same-origin-safe version is a "known daemons" list that *navigates*
(`window.location.href = otherOrigin`) — i.e. bookmarks of authed URLs, each
target re-using its own persisted cookie (`.web-sessions.json` survives daemon
restarts). Not worth building for one developer with one server.

### Layer 2 — Tailscale Serve security posture (the A2 work)

All opt-in, so the daemon's default remains loopback-only and safe.

- **Host allowlist.** `isAllowedHost` accepts only loopback today
  (`src/daemon/web-auth.ts:150`), an anti-DNS-rebind guard we keep. Add an
...
- **`Secure` cookie over TLS.** The bootstrap cookie omits `Secure` today
  because the daemon is plain http on loopback (`src/daemon/server.ts:84`).
  Behind Serve the daemon still sees http but gets `X-Forwarded-Proto: https`.
...
- **CORS: no change.** `denyBrowserCors` only rejects `OPTIONS`
  (`src/daemon/auth.ts:11`); Serve keeps the webapp same-origin with the
  daemon, so no preflight occurs. (A split-origin frontend *would* need real
  CORS — that's out of scope.)
- **No bind change.** Serve → loopback means `src/daemon/cli.ts:100` is
  untouched.

### Layer 3 — Server lifecycle ("close the laptop" for real)

The daemon and cluster must outlive any login session and survive reboots.

- **Daemon as a systemd service.** Today `yaac daemon start` spawns a detached
  process (`src/daemon/cli.ts:495`) — fine for a laptop, not for a server. Ship
  a sample unit running `yaac daemon run` in the foreground with
  `Restart=always`, `WantedBy=multi-user.target`, and the env from Layer 2
  (`YAAC_ALLOWED_HOSTS`, `YAAC_TRUST_PROXY=1`). Now the daemon restarts on
  crash and on boot.
...
yaac forwards a session's dev-server port to the daemon host: it binds the
...
Locally that's seamless — the daemon host *is* your laptop, so
...
- **B (default for Tailscale) — bind forwarders on the tailnet interface.**
  ...
  - *Cross-plan dependency:* `plans/forwarded-port-env.md` injects the actual
    ...
    **host** is the tailnet name, not `localhost`, so that injection must
    ...
    daemon's external-host setting.

- **C (best UX, transport-agnostic) — `yaac port-forward` to the client's
  localhost.** A new CLI command opens a listener on the laptop
  (`localhost:3000`) and tunnels TCP through the daemon's authenticated channel
  into the pod — `kubectl port-forward`, but over the daemon. Stream the TCP
  bytes over a WebSocket endpoint (the PTY bridge is the existing pattern for
  piping a byte stream over a socket — `src/daemon/pty-bridge.ts`).
  - *Pros:* preserves `localhost:<port>` on every device regardless of where
    the daemon runs (great for dev servers with hardcoded `localhost`
...
external host name as a **daemon/server-level** setting, since it's deployment
...
# 3. run the daemon as a service (env: YAAC_ALLOWED_HOSTS, YAAC_TRUST_PROXY=1)
sudo systemctl enable --now yaac-daemon
...
yaac session list                  # talks to the remote daemon
yaac open                          # prints an authed https URL for any device
```

Close the laptop. The daemon and every session keep running on the server.
...
- **Remote is opt-in and default-safe.** With no `YAAC_ALLOWED_HOSTS` /
  `YAAC_TRUST_PROXY` and no `tailscale serve`, the daemon is exactly as
...
- **Tokens are durable and revocable.** A lost laptop is handled by
  `yaac auth token revoke laptop` on the server — no daemon restart, no
...
- **WebSocket idle timeouts.** `/events` has no server heartbeat (`src/daemon/
  ...
  `src/daemon/pty-bridge.ts:166`) is cheap hardening if drops are frequent.
...
- **Build-id skew** between an updated client CLI and the server daemon —
...
1. **Phase 1 — token store + CLI contexts (Layer 1).** Pure code; validate
   ...
   Tailscale yet. Lands the durable-token + `DaemonTarget` refactor + the
