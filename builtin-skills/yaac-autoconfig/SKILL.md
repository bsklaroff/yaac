...
| `bindMounts` | `{hostPath, containerPath, mode}[]` | Mount host dirs into the container. `hostPath` absolute (`$HOME`/`${HOME}` expanded), `containerPath` absolute, `mode` `"ro"`\|`"rw"`. For ad-hoc mounts prefer the `--add-dir` / `--add-dir-rw` create flags. |
...
| `ephemeralModulesPaths` | `string[]` (default `["node_modules"]`) | Paths (relative to `/workspace`) redirected via symlink to per-session storage so package-manager writes don't touch the host worktree. `[]` disables the redirect. |
